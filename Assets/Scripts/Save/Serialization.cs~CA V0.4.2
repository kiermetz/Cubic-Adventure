...
public static class Serialization{
	...
	public static bool LoadT(Chunk chunk, string s) {
		string saveFile = s;
		saveFile += FileName (chunk.pos);
	...
	public static bool SaveWorldBegin(String worldName, int seed, bool plane) {
		if (!Directory.Exists (saveFolderName + "/" + worldName))
			Directory.CreateDirectory (saveFolderName + "/" + worldName);
		
		StreamWriter file = new StreamWriter (saveFolderName + "/" + worldName + "/world.txt");
		file.WriteLine ("World : " + worldName);
		file.WriteLine ("Seed : " + seed);
		file.WriteLine ("Plane : " + plane);
		file.WriteLine ("StoryEvent : " + 0);
		file.Close();
		return true;
	}
	...
	public static string[] LoadWorld(String location) {
		...
		string[] returnLoad = {linesFile[0].Substring(linesFile[0].LastIndexOf(": ") + 2),
			...
			linesFile[3].Substring(linesFile[3].LastIndexOf(": ") + 2)};
		return returnLoad;
	}
}
