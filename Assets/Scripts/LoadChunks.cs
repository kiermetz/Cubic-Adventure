...
public class LoadChunks : MonoBehaviour
{
    ...
    void FindChunksToLoad()
    {
        ...
        WorldPos playerPos = new WorldPos(
            Mathf.FloorToInt(transform.position.x / Chunk.chunkWidth) * Chunk.chunkWidth,
            0,
            Mathf.FloorToInt(transform.position.z / Chunk.chunkWidth) * Chunk.chunkWidth
            );
		...
		if (updateList.Count == 0)
        {
			...
			for (int i = 0; i < chunkPositions.Length; i++)
            {
                ...
                WorldPos newChunkPos = new WorldPos(
                    chunkPositions[i].x * Chunk.chunkWidth + playerPos.x,
                    0,
                    chunkPositions[i].z * Chunk.chunkWidth + playerPos.z
                    );
				...
				for (int x = newChunkPos.x - Chunk.chunkWidth; x <= newChunkPos.x + Chunk.chunkWidth; x += Chunk.chunkWidth)
				{
					for (int z = newChunkPos.z - Chunk.chunkWidth; z <= newChunkPos.z + Chunk.chunkWidth; z += Chunk.chunkWidth)
					{
    ...
    {
        if (world.GetChunk(pos.x, 0, pos.z) == null)
            world.CreateChunk(pos.x, 0, pos.z);
    }
