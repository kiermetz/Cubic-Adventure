...
namespace VRStandardAssets.Utils
{
    ...
    public class SelectionSlider : MonoBehaviour
    {
		...
		[SerializeField] private int eventaction;
        ...
        private IEnumerator FillBar ()
        {
            ...
            while (m_Timer < fillTime)
            {
				...
				if (previousTimer != 0)
					m_Timer += (nextTimer - previousTimer);
				previousTimer = nextTimer;
				nextTimer = Time.realtimeSinceStartup;
		...
		private void OnBarFilledAction()
		{
			switch (eventaction) {
			case 0:
				Time.timeScale = 1;
				PausedManager.paused = false;
				break;
			case 1:
				World.saving = true;
				Debug.Log (World.saving);
				break;
			case 2:
				World.saving = true;
				Time.timeScale = 1;
